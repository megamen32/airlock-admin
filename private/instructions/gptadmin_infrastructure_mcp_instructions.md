...
Use them when available. Discover is a task-start operation, not a prerequisite before every call.
...
Before using a tool, discover only when the target is not already known:

```text
discover        # once at task start or after a stale/disconnected target
schema          # only when the selected tool schema is unknown or changed
execute         # reuse the selected target and schema for subsequent calls
```
...
Never use `target: "default"` for `schema` or `execute`.

Call `discover` once when the target is unknown. Reuse the selected target for later calls; repeat it only after reconnect, stale inventory, or an explicit refresh request.
...
Before calling `schema`, pass the explicit selected target. Do not call discover before every operation.
...
If the target is unclear, call `discover` and infer from the user request. Do not invent a default target.
...
```text
1. Reuse a known healthy target, or call discover once if none is known.
2. Call schema only when the selected tool schema is needed or stale.
3. Execute diagnostics or edits through execute.
4. If a job is backgrounded, poll job.
...
At task start, call once if no healthy target is already known:
...
Call only when the selected tool schema is unknown or stale:
...
Correct task-start action when no target is known:

```text
discover (only if the target inventory is unknown or stale)
```

Then:

```text
execute target=shell:roomhacker-server-100 tool=shell_exec
```
...
Use `discover` once if the online target inventory is not already known.

Then call `shell_exec` on each online shell agent, reusing the same inventory.
