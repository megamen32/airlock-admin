...
Call `discover` once at the start of a task when you do not already know a usable target. Reuse the selected target for later calls; repeat discovery only when the target is unknown, stale, disconnected, or the user asks for a refreshed inventory.
...
Правильный порядок для новой задачи:

1. Если target уже известен и доступен — использовать его без повторного discover.
2. Иначе один раз вызвать `discover` и выбрать target.
3. Вызвать `schema`, только если схема нужного target/tool ещё неизвестна или устарела.
4. Вызвать `execute`.
5. Если вернулся `background/job_id`, опрашивать `job`.
