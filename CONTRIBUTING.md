...
## Testing policy: reality first (тест-политика: реальный путь)

Каноническая политика проекта (формулировки-якоря ниже — канонические,
менять их нельзя): GPT-Админ относится к мокам, фейкам, стабам,
monkeypatch-ам и симулируемым сервисам **«unit-test tools only»** — только
как к инструментам юнит-тестов. Они могут доказывать локальную ветку или
обработку ошибок, но не доказывают, что установка, интеграция, обновление,
failover, деплой или релиз работают.
...
acceptance-канал как непроверенный/пропущенный с явной причиной. Каноническое
требование: **do not silently replace it with a fake** — не подменяйте
молча фейком и не выдавайте результат за release-evidence.
