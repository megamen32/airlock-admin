...
| Milestone | Deliverable | Exit gate | Status |
| --- | --- | --- | --- |
| S4.1 Integration certification and control contract | Certify the existing `list_mcp_agents/servers -> list_mcp_tools -> call_mcp_tool` flow and define adapter rules for session-oriented integrations | Every listed integration has automated smoke evidence and a known-version support policy; write retries have explicit idempotency semantics where the current call contract lacks them | Planned |
