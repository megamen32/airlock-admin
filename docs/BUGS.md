...
## 2026-07-23 - WIN-ROOTD-CERT-20260723 - Windows bundled CA path drift - in_progress
...
- Fix / verification: The active user-mode path now uses the Go binary and no longer uses the stale PyInstaller bundle. The legacy system task still needs elevated cleanup; its historical log remains the evidence artifact.
- Next action: Remove or disable the inaccessible legacy system task from an elevated Windows session, then archive this entry as fixed after a fresh restart check.
