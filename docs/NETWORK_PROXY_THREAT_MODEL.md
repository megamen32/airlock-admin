...
| Webhook delivery injects a target, ticket, or payload. | Webhook contains only signed `offer_id`, timestamp, and nonce. The agent verifies it and uses only `/proxy-agent/v1/offers` to retrieve an offer; it never uses `/queue/{server}`. |
