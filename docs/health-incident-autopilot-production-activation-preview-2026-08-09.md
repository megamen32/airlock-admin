...
|---|---|---|
| server-100 | `host:roomhacker-server-100` | local target; use the local producer or central adapter, not both |
| roomhacker-server-88 | `host:roomhacker-server-88` | Linux target; local producer or central SSH adapter |
| server-44 | `host:server-44` | Linux target; local producer or central SSH adapter |
...
| vpn2 | `host:vpn2` | central SSH adapter or local producer; current probe shows load violation |
| vusa | `host:vusa` | central SSH adapter or local producer; current probe shows disk at 85% |
...
| roomhacker-server-88 | load `65.98` / 88 CPUs | 82% | 48,067 MiB | 1 (`certbot.service`) | journal/files |
| server-44 | load `54.83` / 88 CPUs | 59% | 30,605 MiB | 1 (`gptadmin-tunnel-frpc.service`) | journal |
| router | load `6.51` / 4 proc fallback | 25% | 11 MiB | n/a | `logread` |
| haos | load `0.85` / 4 CPUs | 68% | 1,535 MiB | n/a | `logread` |
| vpn2 | load `50.83` / 1 CPU | 79% | 413 MiB | 1 (`coturn.service`) | journal/files |
| vusa | load `0.00` / 1 CPU | 85% | 464 MiB | 0 | journal/files |
...
filtered to `vpn2` emitted a `router` recovery because its state file is global
to all hosts. This is recorded in
`/home/roomhacker/gptadmin/.agents/tasks/todo-20260809-fleet-health-dry-run-cross-host-state.md`;
the new producer must not reuse that filtered state path until reconciled.

The source-level implementation remains in:

- `/home/roomhacker/ServersAdministartion/automation/health-incident-monitor/`
- `/home/roomhacker/agents-projects/noticeplace/`
- `/home/roomhacker/gptadmin/go-hub/`
- `/home/roomhacker/agents-projects/agent-herder/`
- `/home/roomhacker/agents-projects/hermes-config/plugins/agent-herder-bridge/`
- `/home/roomhacker/agents-projects/agent-resume/` (compatibility fix: stop
...
`/home/roomhacker/agents-projects/agent-herder/dist.backup-20260809T154620Z`.
...
`root:roomhacker`; its values are non-secret.
