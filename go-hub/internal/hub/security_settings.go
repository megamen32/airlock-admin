...
func defaultSecuritySettings() securitySettings {
	return securitySettings{Preset: securityPresetWorkingDefault, Process: defaultProcessSecurityProfile()}
}
...
func loadSecuritySettings(path, key string) (securitySettings, error) {
	...
	state = securitySettings{Preset: persisted.Preset, Process: persisted.Process, MFAEnrolledAt: persisted.MFAEnrolledAt, RecoveryCodeHashes: persisted.RecoveryCodeHashes, UpdatedAt: persisted.UpdatedAt}
...
func saveSecuritySettings(path string, state securitySettings, key string) error {
	...
	persisted := persistedSecuritySettings{
		Preset: state.Preset, Process: state.Process, EncryptedTOTP: encrypted, MFAEnrolledAt: state.MFAEnrolledAt,
...
func (s *Server) adminSecurityProfile(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		writeJSON(w, http.StatusOK, map[string]any{"process_profile": s.securitySnapshot().Process})
	case http.MethodPut:
		if !s.requireSensitiveSecurityReauth(w, r) {
			return
		}
		var profile processSecurityProfile
		...
		s.addSecurityAudit("process_security_profile_changed", map[string]any{"mode": profile.Mode})
		writeJSON(w, http.StatusOK, map[string]any{"process_profile": profile, "restart_bound": true})
	default:
