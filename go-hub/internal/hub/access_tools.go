...
func accessProfileTool() map[string]any {
	...
	return map[string]any{"name": "access_profiles", "description": "Owner API: list/get/create/update access profiles. Read before update and pass returned etag. Existing browser/CTL authorization applies; no shell required.", "inputSchema": map[string]any{"type": "object", "properties": map[string]any{"action": map[string]any{"type": "string", "enum": []string{"list", "get", "create", "update"}}, "id": str, "etag": str, "profile": profile}, "required": []string{"action"}, "additionalProperties": false}, "annotations": map[string]any{"readOnlyHint": false, "destructiveHint": false, "openWorldHint": false}}
}
