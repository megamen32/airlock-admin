...
func TestHubMCP20260728TasksLifecycle(t *testing.T) {
	s := New(Config{CtlToken: "ctl", DefaultTimeout: 1, PollMaxTimeout: 1})
	s.mu.Lock()
	s.shellJobs["task-1"] = &shellJob{ID: "task-1", Server: "roomhacker-server-100", CreatedAt: nowFloat(), Status: "queued", Cmd: "sleep 60"}
	s.shellQueues["roomhacker-server-100"] = append(s.shellQueues["roomhacker-server-100"], "task-1")
	s.mu.Unlock()
	...
	if firstString(progress, "stage") != "working" || firstString(progress, "target") != "shell:roomhacker-server-100" {
