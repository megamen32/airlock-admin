...
def _maybe_add_headroom_path() -> None:
    ...
    candidates.extend(Path("/home/roomhacker/.venvs/headroom/lib").glob("python*/site-packages"))
...
def _shell_cli_info(srv: str, info: Dict[str, Any], verbose: bool = False) -> Dict[str, Any]:
    ...
    if srv == "roomhacker-server-100":
        info_out["repo_helper"] = "/home/roomhacker/gptadmin/mcp-add NAME -- npx -y package"
    return info_out
...
def _shell_help(srv: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    is_hub_host = srv == _socket_module.gethostname() or srv == "roomhacker-server-100"
...
def _fmt_ts(value: Any) -> Optional[str]:
    ...
    if ts <= 0:
        return None
    # Hub timezone is the server local timezone; currently MSK on roomhacker-server-100.
