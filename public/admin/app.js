  ...
  return `<article class="entryCard" style="cursor:pointer" onclick="openJobDetail('${esc(row.job_id||'')}','${esc(row.server||row.agent_id||'hub')}')">
    <div class="entryHead">
      ...
      <div class="entryMain">
        ...
        <div class="entryMeta">
          ${row.kind?`<span class="pill">${esc(row.kind)}</span>`:''}
      ...
      '<div class="entryHead">' +
        ...
        '<div class="entryMain">' +
          ...
          '<div class="entrySub muted small">' +
            (r.kind ? '<span class="pill">' + esc(r.kind) + '</span> ' : '') +
...
async function issueMcpTokenFromPanel(){
  ...
  try{
    const cmd=`cd /home/roomhacker/gptadmin 2>/dev/null && python3 cli.py mcp token '${name.replace(/'/g,"'\''")}' --no-save 2>&1 || echo FALLBACK`;
    const j=await api('/mcp-relay/call',{method:'POST',body:JSON.stringify({
      target:'shell:roomhacker-server-100',
      tool_name:'shell_exec',
      arguments:{cmd}
...
function openAgentDetail(aid){
  ...
  h+='<div class="entryCard"><div class="entryHead"><span class="entryStatus pill '+cls(a.status)+'">'+esc(a.status)+'</span><div class="entryMain"><div class="entryTitle"><b class="mono">'+esc(a.agent_id)+'</b>'+(a.name?'<span class="muted small">'+esc(a.name)+'</span>':'')+'</div><div class="entryMeta"><span class="pill">'+esc(a.kind||'?')+'</span><span class="muted small">'+esc(a.transport||'')+'</span></div></div></div></div>';
