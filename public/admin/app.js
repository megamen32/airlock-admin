...
function renderServerCard(r) {
  ...
  const capFirst = topN(caps, SERVER_CARD_CAPS_SHOWN);
  const metaFirst = topN(metaKeys, SERVER_CARD_META_KEYS_SHOWN);
  const capMore = Math.max(0, caps.length - SERVER_CARD_CAPS_SHOWN);
  const metaMore = Math.max(0, metaKeys.length - SERVER_CARD_META_KEYS_SHOWN);
  ...
  const capRest = caps.slice(SERVER_CARD_CAPS_SHOWN).map(x => '<li><span class="pill mono small">' + esc(x) + '</span></li>').join('');
  const metaList = metaFirst.map(k => '<li><span class="mono">' + esc(k) + '</span>: <span class="muted">' + esc(metaValueForList(meta[k])) + '</span></li>').join('');
  const metaRest = metaKeys.slice(SERVER_CARD_META_KEYS_SHOWN).map(k => '<li><span class="mono">' + esc(k) + '</span>: <span class="muted">' + esc(metaValueForList(meta[k])) + '</span></li>').join('');
  return (
...
async function loadHubSettings(){try{const j=await api('/admin/api/settings');const v=j.settings?.stale_mcp_retention_days;if($('staleMcpRetentionDays')&&v)$('staleMcpRetentionDays').value=v;if($('hubSettingsResult'))$('hubSettingsResult').textContent=JSON.stringify(j,null,2)}catch(e){if($('hubSettingsResult'))$('hubSettingsResult').textContent='ERR '+e.message}}
async function saveHubSettings(){try{const days=+$('staleMcpRetentionDays').value;const j=await api('/admin/api/settings',{method:'PUT',body:JSON.stringify({stale_mcp_retention_days:days})});$('hubSettingsResult').textContent=JSON.stringify(j,null,2);await refreshAll()}catch(e){$('hubSettingsResult').textContent='ERR '+e.message}}
