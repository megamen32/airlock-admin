...
- Final real browser canary passed through the full path: BrowserClaw via GPTAdmin opened the private Admin Custom GPT, entered `uptime`, ChatGPT displayed the explicit Action consent for `shell:roomhacker-server-100`, consent was accepted, and the assistant returned `roomhacker-server-100: 17:22:18 up 1 day, 8:08, 1 user` plus load averages. The response was read from the rendered ChatGPT page via BrowserClaw, not from Hub internals.
- Post-canary GPTAdmin discovery shows `hub`, `shell:roomhacker-server-100`, `shell:mac-mini-2012.lan`, `BrowserOS on roomhacker-server-100`, and `mcp:shell:mac-mini-2012.lan:mac-mini-browserclaw` online. Child status reports BrowserClaw enabled/running with PID 19817 and the corrected 9010 endpoint.
- Public authoritative DNS now returns only `95.165.165.65` for the canonical hostname; direct HTTPS `--resolve` to that address returns HTTP 200 and build 147. The live certificate is the newly issued public certificate for the canonical hostname, replacing the incident-fallback certificate. Source TLS-root handling fix is pushed as `f9d4cf3` and deployed to Mac ShellMCP.
