...
Scope checked: only `/home/roomhacker/gptadmin/grepmesh` source, tests, README,
...
- Owned path only: `/home/roomhacker/gptadmin/grepmesh/src/backend.rs`.
...
- `cargo clippy --manifest-path grepmesh/Cargo.toml --all-targets -- -D
  warnings`: PASS.
- `cargo fmt --manifest-path grepmesh/Cargo.toml --all -- --check`: PASS.
- `go test ./...` in `/home/roomhacker/gptadmin/go-hub`: PASS; the Go working
