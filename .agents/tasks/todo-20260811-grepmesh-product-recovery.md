...
- Production roots must use `/home/roomhacker`, not all `/home`; `/opt` and `/etc` are explicit named roots.
...
- ShellMCP preflight confirmed free `9419` on both targets. server-100 UFW already allows source `192.168.2.75`; server-88 input policy is accept. No firewall mutation is required for the first slice.
- server-88 `/home/roomhacker` is `0750`; the committed unit uses `SupplementaryGroups=roomhacker`, so the service can traverse only group-readable project paths. Private files stay excluded and unreadable unless their existing mode permits otherwise.
