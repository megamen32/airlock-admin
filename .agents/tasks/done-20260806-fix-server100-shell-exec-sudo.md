...
Предыдущий read-only аудит подтвердил, что на `shell:roomhacker-server-100` любой `shell_exec` падает до запуска команды: `fork/exec /usr/bin/sudo: no such file or directory`.
...
`gptadmin_execute(target=shell:roomhacker-server-100, tool=shell_exec, cmd=id)` возвращает `returncode=0`, ожидаемого non-root пользователя и пустой stderr.
...
- RED: `gptadmin_execute(shell:roomhacker-server-100, shell_exec, id)` failed with `fork/exec /usr/bin/sudo: no such file or directory`.
...
- Live service: `active`, `MainPID=3428021`, `User=roomhacker`, `Group=roomhacker`.
- Live GPTADMIN canary: `shell_exec(id -un && pwd)` returned `roomhacker` and `/home/roomhacker`, returncode `0`.
- Live `system_inspect(list_directory, /home/roomhacker)` passed; home is visible while `ProtectHome=read-only` remains enforced.
- Phone proxy `192.168.2.100:3122` accepted TCP but HTTPS CONNECT aborted (`curl 000`, `Proxy CONNECT aborted`). VPN2 external canaries passed: `example.com` HTTP `200`; `httpbin.org/status/204` HTTP `204`.
