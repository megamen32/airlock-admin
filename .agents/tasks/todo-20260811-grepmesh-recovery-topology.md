...
|---|---|---|---|---|---|
| server-100 | `roomhacker-server-100` -> `192.168.2.100:22104`, user `roomhacker` | Ubuntu kernel `6.8.0-136-generic`, x86_64; `enp28s0f2np2 UP 192.168.2.100/24 192.168.1.100/24`; other physical NICs down | `gptadmin-hub.service=active`, `gptadmin-tunnel-frpc.service=active`; canonical binary path `/opt/gptadmin/bin`; systemd available | No `grepmesh-mcp.service` listed/probed; no `:9419` listener | `roomhacker` has `sudo NOPASSWD: ALL`; `:9001` listens on `192.168.2.100` and `127.0.0.1`; `/opt/gptadmin` and `/usr/local/bin` exist |
| server-88 | `roomhacker-server-88` -> `192.168.2.75:22`, user `roomhacker` | Ubuntu kernel `6.8.0-136-generic`, x86_64; `enp6s0 UP 192.168.2.75/24 192.168.1.75/24` | `shellmcp.service=active`, `User=root`, `ExecStart=/opt/gptadmin/bin/rootd-go`; systemd available | `grepmesh-mcp.service=inactive` (unit is not installed/listed); no `:9419` listener | `roomhacker-search` does not resolve via `getent passwd`; `/opt/gptadmin` and `/usr/local/bin` exist |

### Transport and topology findings

- SSH aliases are configured in `/home/roomhacker/.ssh/config:24-31`; no alternate management/VPN hop is required for either target.
- Cross-host TCP probes from 100 -> `192.168.2.75:9419` and 88 -> `192.168.2.100:9419` both returned connection refused. This is consistent with no service/listener, not proof of a firewall allow rule.
...
- `grepmesh/grepmesh-mcp.service:8-18` requires a dedicated `roomhacker-search` identity, `/usr/local/bin/grepmesh-mcp`, `/etc/grepmesh-mcp/config.json`, and writable `/var/lib/grepmesh-mcp`; the dedicated identity is absent on server-88 and was not proven on server-100.
...
3. Configure server-100 as `host_id=server-100`, bind `192.168.2.100:9419`; configure server-88 as `host_id=server-88`, bind `192.168.2.75:9419`; use exact peer URLs and an approved auth/mTLS/ACL policy.
