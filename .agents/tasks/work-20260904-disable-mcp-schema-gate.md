...
Status: deployed. The prior binary is `/opt/gptadmin/backups/mcp-schema-gate-20260904-0207/gptadmin_hub.before`; `gptadmin-hub.service` is active. A stale-digest REST relay call and Fast-Agent CLI MCP call both ran `shell_exec hostname` successfully on `roomhacker-server-100` without schema fields.
