...
- После двух polling без новой фазы допустим один короткий non-interrupting
  heartbeat. Silence не является blocker: продолжайте ждать исходный budget.
  После budget допускается один read-only discriminating probe; не меняйте
  задачу посреди цикла без необходимости.
