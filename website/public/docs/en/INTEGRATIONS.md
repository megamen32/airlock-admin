...
**Protocol.** REST + OpenAPI 3.1, Bearer auth. The compact flow is `discover → schema → execute`; `job` polls background work. Legacy long names remain accepted but are not advertised.
