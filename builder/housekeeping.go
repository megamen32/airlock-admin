...
func (r HousekeepingResult) Changed() bool {
	return r.DockerfileChanged || r.AgentsMDChanged || r.GitignoreChanged || r.GoModChanged || r.NoticesChanged || len(r.GeneratedFilesRemoved) > 0
}
...
var gitignoreManagedLines = append([]string{
	"go.work",
	"go.work.sum",
	".airlock/local/",
	".airlock/toolchain/",
}, scaffold.GeneratedArtifactIgnorePatterns()...)
...
//     entries untouched
//   - generated artifacts: tracked outputs covered by the scaffold policy are
//     removed because builds recreate them from committed source inputs
...
func runHousekeeping(ctx context.Context, repoPath string, data scaffold.ScaffoldData) (HousekeepingResult, error) {
	...
	removed, err := removeTrackedGeneratedArtifacts(repoPath)
	if err != nil {
		return res, fmt.Errorf("remove tracked generated artifacts: %w", err)
	}
	res.GeneratedFilesRemoved = removed
	...
	before := goModBody
	if err := reconcileAgentGoMod(ctx, repoPath, data.AgentSDKVersion); err != nil {
		return res, fmt.Errorf("reconcile go.mod: %w", err)
	}
...
func removeTrackedGeneratedArtifacts(repoPath string) ([]string, error) {
	...
	for _, rel := range strings.Split(tracked, "\n") {
		...
		if !scaffold.IsGeneratedArtifact(filepath.ToSlash(rel)) {
			continue
		}
...
func commitHousekeeping(repoPath string, r HousekeepingResult) error {
	...
	paths = append(paths, r.GeneratedFilesRemoved...)
	if len(paths) == 0 {
		return nil
	}
