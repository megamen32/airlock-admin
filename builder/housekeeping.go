...
var gitignoreManagedLines = []string{
	"go.work",
	"go.work.sum",
...
//     entries untouched
//   - generated templ output: tracked files carrying templ's generated-file
//     header are removed because builds regenerate them from .templ source
...
func runHousekeeping(ctx context.Context, repoPath string, data scaffold.ScaffoldData) (HousekeepingResult, error) {
	...
	removed, err := removeTrackedTemplOutput(repoPath)
	if err != nil {
		return res, fmt.Errorf("remove generated templ output: %w", err)
	}
...
func removeTrackedTemplOutput(repoPath string) ([]string, error) {
	...
	for _, rel := range strings.Split(tracked, "\n") {
		...
		if !strings.HasSuffix(filepath.ToSlash(rel), "_templ.go") {
			continue
		}
		...
		if !bytes.HasPrefix(body, []byte("// Code generated by templ - DO NOT EDIT.\n")) {
			continue
		}
