...
func TestRunHousekeeping_AppendsMissingGitignoreLines(t *testing.T) {
	...
	for _, want := range []string{"go.work", "go.work.sum", "views/static/app.css", "*_templ.go", ".airlock/local/", ".airlock/toolchain/", "build/", "# my stuff"} {
...
func TestRunHousekeeping_RemovesTrackedTemplOutput(t *testing.T) {
	ctx := context.Background()
	repoPath, data := scaffoldHousekeepingFixture(t)
	generated := filepath.Join(repoPath, "views", "index_templ.go")
	if err := os.WriteFile(generated, []byte("// Code generated by templ - DO NOT EDIT.\n\npackage views\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := git(repoPath, "add", "-f", "--", "views/index_templ.go"); err != nil {
		t.Fatal(err)
	}
	...
	if len(res.GeneratedFilesRemoved) != 1 || res.GeneratedFilesRemoved[0] != "views/index_templ.go" {
		t.Fatalf("GeneratedFilesRemoved = %#v", res.GeneratedFilesRemoved)
	}
	if _, err := os.Stat(generated); !os.IsNotExist(err) {
		t.Fatalf("generated templ output still exists: %v", err)
	}
	if err := commitHousekeeping(repoPath, res); err != nil {
		t.Fatalf("commitHousekeeping: %v", err)
	}
	tracked, err := gitOutput(repoPath, "ls-files", "--", "views/index_templ.go")
	if err != nil {
		t.Fatal(err)
	}
	if tracked != "" {
		t.Fatalf("generated templ output remains tracked: %s", tracked)
	}
}
