...
export default function FailoverScreen() {
  ...
  return <div className="page-shell native-operation-page">
    <header className="page-header compact-page-header"><div><p className="section-kicker">FAILOVER</p><h1>Резервирование</h1><p className="lede">Только настройки promotion и fallback-узлов; state bundle вынесен в сворачиваемую диагностику.</p></div><div className="overview-header-actions"><button className="button secondary" type="button" disabled={busy} onClick={() => void load()}>Обновить</button><button className="button primary" type="button" disabled={busy} onClick={() => void save()}>Сохранить</button></div></header>
    ...
    <section className="card failover-main-card">
      <div className="failover-main-settings"><label className="switch-line"><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} /><span><strong>Self-host failover</strong><small>Разрешить promotion fallback-узлов</small></span></label><label><span>Primary public URL</span><input value={primary} onChange={(event) => setPrimary(event.target.value)} placeholder="https://..." /></label><label><span>Fail count base</span><input type="number" min={1} value={base} onChange={(event) => setBase(Number(event.target.value) || 3)} /></label></div>
      <div className="failover-node-list">{servers.length === 0 ? <div className="compact-empty"><strong>Shell-серверов нет</strong><span>Fallback можно настроить после регистрации shell-узла.</span></div> : servers.map((server) => { const id = server.server_id || ""; const node = nodes[id] ?? { server_id: id, enabled: false, rank: 1, hub_url: "" }; return <article className="failover-node-row" key={id}><label className="switch-line"><input type="checkbox" checked={node.enabled} onChange={(event) => updateNode(id, { enabled: event.target.checked })} /><span><strong>{id}</strong><small>{server.status || "unknown"}</small></span></label><label><span>Rank</span><input type="number" min={1} value={node.rank} onChange={(event) => updateNode(id, { rank: Number(event.target.value) || 1 })} /></label><label className="hub-url-field"><span>Local Hub URL</span><input value={node.hub_url} onChange={(event) => updateNode(id, { hub_url: event.target.value })} placeholder="http://192.168.2.x:9001" /></label></article>; })}</div>
    </section>
    <details className="card operation-result"><summary>State bundle</summary><pre className="raw-native-box">{state === null ? "—" : JSON.stringify(state, null, 2)}</pre></details>
  </div>;
}
