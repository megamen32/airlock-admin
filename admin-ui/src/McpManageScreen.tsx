...
export default function McpManageScreen() {
  ...
  const add = async () => {
    ...
    if (!draft.url.trim() && !draft.command.trim()) { setError("Укажите Remote URL или Command"); return; }
  ...
  return <div className="page-shell native-operation-page">
    <header className="page-header compact-page-header"><div><p className="section-kicker">MCP MANAGEMENT</p><h1>MCP</h1><p className="lede">Управление MCP-сервисами и retention без отдельной legacy-консоли.</p></div><span className={`data-badge ${error ? "state-error" : "state-ready"}`} role="status">{busy ? "Выполняем…" : hostLabel?.status || "готово"}</span></header>
    {error && <div className="state-panel card standalone-state state-error" role="alert">{error}</div>}
    <section className="card mcp-retention-card"><div><p className="section-kicker">CLEANUP</p><h2>Очистка stale MCP</h2><p className="muted-help">Shell-серверы не затрагиваются.</p></div><div className="inline-setting"><input type="number" min={1} max={3650} value={retention} onChange={(event) => setRetention(Number(event.target.value) || 30)} /><span>дней</span><button className="button secondary" type="button" disabled={busy} onClick={() => void saveRetention()}>Сохранить</button></div></section>
    <section className="mcp-native-layout">
      <article className="card mcp-list-card">
        <div className="card-heading"><div><p className="section-kicker">SERVICES</p><h2>Установленные MCP</h2></div><div className="mcp-list-actions"><select value={host} onChange={(event) => { setHost(event.target.value); setRows([]); }}>{hosts.map((item) => <option key={item.server_id} value={item.server_id}>{item.server_id} ({item.status || "—"})</option>)}</select><button className="button secondary" type="button" disabled={busy} onClick={() => void manage({ target: host, action: "list" })}>Список</button><button className="button secondary" type="button" disabled={busy} onClick={() => void manage({ target: host, action: "status", ...(draft.backend ? { backend: draft.backend } : {}) })}>Статус</button></div></div>
        <label className="keep-service-line"><input type="checkbox" checked={keepService} onChange={(event) => setKeepService(event.target.checked)} /> При remove оставить service definition</label>
        {rows.length === 0 ? <div className="compact-empty"><strong>MCP не загружены</strong><span>Нажмите «Список» для выбранного узла.</span></div> : <div className="mcp-service-list">{rows.map((row) => <article className="mcp-service-row" key={row.name || row.server_id}><div><strong>{row.name || "Без имени"}</strong><small>{row.server_id || row.command || row.url || "—"}</small><div className="pill-wrap"><span className={`small-pill ${row.enabled === false ? "is-disabled" : ""}`}>{row.enabled === false ? "disabled" : "enabled"}</span>{row.stdio_format && <span className="small-pill">{row.stdio_format}</span>}</div></div><div className="row-actions"><button className="text-button" type="button" disabled={busy} onClick={() => void manage({ target: host, action: "status", name: row.name, ...(draft.backend ? { backend: draft.backend } : {}) })}>status</button><button className="text-button" type="button" disabled={busy} onClick={() => void manage({ target: host, action: "install", name: row.name, ...(draft.backend ? { backend: draft.backend } : {}) }, true)}>install</button><button className="text-button danger-text" type="button" disabled={busy} onClick={() => { if (window.confirm(`Удалить MCP ${row.name} на ${host}?`)) void manage({ target: host, action: "remove", name: row.name, keep_service: keepService, ...(draft.backend ? { backend: draft.backend } : {}) }, true); }}>remove</button></div></article>)}</div>}
      </article>
      <article className="card mcp-add-card"><div className="card-heading"><div><p className="section-kicker">ADD</p><h2>Добавить MCP</h2></div></div><div className="mcp-form-grid">
        ...
        <label><span>Server ID</span><input value={draft.serverId} onChange={(event) => setDraft({ ...draft, serverId: event.target.value })} placeholder="опционально" /></label>
        <label className="span2"><span>Remote URL</span><input value={draft.url} onChange={(event) => setDraft({ ...draft, url: event.target.value })} placeholder="https://.../mcp" /></label>
        <label className="span2"><span>Command</span><input value={draft.command} onChange={(event) => setDraft({ ...draft, command: event.target.value })} placeholder="npx / python3" /></label>
        <label><span>Args JSON</span><textarea rows={4} value={draft.args} onChange={(event) => setDraft({ ...draft, args: event.target.value })} /></label>
        <label><span>Env JSON</span><textarea rows={4} value={draft.env} onChange={(event) => setDraft({ ...draft, env: event.target.value })} /></label>
        <label><span>Run as</span><input value={draft.runAs} onChange={(event) => setDraft({ ...draft, runAs: event.target.value })} /></label>
        <label><span>Backend</span><select value={draft.backend} onChange={(event) => setDraft({ ...draft, backend: event.target.value })}><option value="">auto</option><option value="systemd">systemd</option><option value="launchd">launchd</option><option value="windows-task">windows-task</option></select></label>
        <label><span>stdio</span><select value={draft.stdio} onChange={(event) => setDraft({ ...draft, stdio: event.target.value })}><option value="">auto</option><option value="ndjson">ndjson</option><option value="jsonl">jsonl</option><option value="framed">framed</option><option value="content-length">content-length</option></select></label>
        <div className="mcp-checks"><label><input type="checkbox" checked={draft.install} onChange={(event) => setDraft({ ...draft, install: event.target.checked })} /> install</label><label><input type="checkbox" checked={draft.force} onChange={(event) => setDraft({ ...draft, force: event.target.checked })} /> force</label><label><input type="checkbox" checked={draft.disabled} onChange={(event) => setDraft({ ...draft, disabled: event.target.checked })} /> disabled</label></div>
      </div><button className="button primary full-button" type="button" disabled={busy} onClick={() => void add()}>Добавить MCP</button></article>
    </section>
    {result !== null && <details className="card operation-result"><summary>Последний ответ Hub</summary><pre className="raw-native-box">{JSON.stringify(result, null, 2)}</pre></details>}
  </div>;
}
