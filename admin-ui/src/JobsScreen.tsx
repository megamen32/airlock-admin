...
export default function JobsScreen() {
  ...
  return <div className="page-shell native-operation-page">
    <header className="page-header compact-page-header"><div><p className="section-kicker">JOBS</p><h1>Задачи</h1><p className="lede">Очередь, история и полный результат выполнения в одном экране.</p></div><div className="overview-header-actions"><span className={`data-badge ${error ? "state-error" : "state-ready"}`} role="status">{loading ? "Обновляем…" : `${jobs.length} / ${total}`}</span><button className="button secondary" type="button" onClick={() => void load()} disabled={loading}>Обновить</button></div></header>
    ...
    <section className={`job-native-layout ${selected ? "has-detail" : ""}`}>
      <article className="card job-native-list">
        <div className="list-toolbar"><input className="search" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Фильтр задач" /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Все статусы</option><option value="running">running</option><option value="queued">queued</option><option value="completed">completed</option><option value="failed">failed</option></select></div>
        <div className="compact-list">{filtered.map((job) => <button className={`job-native-row ${selected?.job_id === job.job_id ? "selected" : ""}`} type="button" key={job.job_id || `${job.created_at}-${job.tool_name}`} onClick={() => void open(job)}><span className={`job-state job-${job.status || "unknown"}`}>{job.status || "—"}</span><span className="job-native-main"><strong>{job.tool_name || "operation"}</strong><small>{job.server_id || job.server || "hub"} · {compactTime(job.created_at)}</small><code>{jobInput(job)}</code>{jobPreview(job) && <em>{jobPreview(job)}</em>}</span></button>)}</div>
      ...
      {selected && <aside className="card job-detail-pane"><div className="card-heading"><div><p className="section-kicker">JOB DETAIL</p><h2>{selected.job_id || "Задача"}</h2></div><button className="text-button" type="button" onClick={() => { setSelected(null); setDetail(null); }}>Закрыть</button></div><dl className="detail-facts"><div><dt>Статус</dt><dd>{selected.status || "—"}</dd></div><div><dt>Tool</dt><dd>{selected.tool_name || "—"}</dd></div><div><dt>Server</dt><dd>{selected.server_id || selected.server || "hub"}</dd></div><div><dt>Создана</dt><dd>{compactTime(selected.created_at)}</dd></div></dl><div className="detail-section"><h3>Входные данные</h3><pre className="detail-code">{jobInput(selected)}</pre></div><div className="detail-section"><h3>Полный результат</h3><pre className="raw-native-box job-full-result">{detailLoading ? "Загрузка…" : detail === null ? "—" : JSON.stringify(detail, null, 2)}</pre></div></aside>}
    </section>
  </div>;
}
