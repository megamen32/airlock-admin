...
export default function SecurityScreen() {
  ...
  const enrollTotp = async () => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : "MFA enrollment не выполнен"); }
    finally { setBusy(false); }
  };
  const verifyTotp = async () => {
    if (!mfaCode.trim()) { setError("Введите MFA-код"); return; }
    setBusy(true); setError(null);
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : "MFA-код не подтверждён"); }
    finally { setBusy(false); }
  };
  ...
  return <div className="page-shell native-operation-page">
    <header className="page-header compact-page-header"><div><p className="section-kicker">SECURITY</p><h1>Безопасность</h1><p className="lede">Редкие административные настройки собраны в одном компактном месте.</p></div><div className="overview-header-actions"><button className="button secondary" type="button" disabled={busy} onClick={() => void load()}>Обновить</button><button className="button danger" type="button" disabled={busy} onClick={() => void rotateOAuth()}>Ротировать OAuth</button></div></header>
    ...
    <section className="security-native-grid">
      <article className="card security-section"><div className="card-heading"><div><p className="section-kicker">PROFILE</p><h2>Профиль безопасности</h2></div><span className={`small-pill ${state.mfaEnrolled ? "" : "is-disabled"}`}>MFA {state.mfaEnrolled ? "enrolled" : "off"}</span></div><div className="security-body"><select value={state.preset} onChange={(event) => setState({ ...state, preset: event.target.value })}><option value="working_default">Working default</option><option value="private_access">Private access</option><option value="locked_down">Locked down</option></select><button className="button primary" type="button" disabled={busy} onClick={() => void savePreset()}>Сохранить профиль</button><p className="muted-help">Locked down требует подтверждённой MFA-регистрации.</p></div></article>
      <article className="card security-section"><div className="card-heading"><div><p className="section-kicker">MFA</p><h2>Администратор</h2></div></div><div className="security-body"><div className="security-inline"><button className="button secondary" type="button" disabled={busy} onClick={() => void enrollTotp()}>Начать TOTP</button><input inputMode="numeric" value={mfaCode} onChange={(event) => setMfaCode(event.target.value)} placeholder="6-значный код" /><button className="button primary" type="button" disabled={busy} onClick={() => void verifyTotp()}>Подтвердить</button></div><button className="button secondary" type="button" disabled={busy} onClick={() => void enrollPasskey()}>Зарегистрировать passkey</button>{mfaResult !== null && <pre className="raw-native-box compact-result">{JSON.stringify(mfaResult, null, 2)}</pre>}{passkeyResult !== null && <pre className="raw-native-box compact-result">{JSON.stringify(passkeyResult, null, 2)}</pre>}</div></article>
      <article className="card security-section"><div className="card-heading"><div><p className="section-kicker">LOCAL</p><h2>Диагностика</h2></div></div><div className="security-body"><label className="switch-line"><input type="checkbox" checked={state.heartbeat} onChange={(event) => void toggleHeartbeat(event.target.checked)} /><span><strong>Дополнительный heartbeat</strong><small>По умолчанию выключен</small></span></label><label className="switch-line"><input type="checkbox" checked={state.telemetry} onChange={(event) => void toggleTelemetry(event.target.checked)} /><span><strong>Локальная телеметрия</strong><small>Данные не отправляются наружу</small></span></label>{telemetryInfo && <pre className="raw-native-box compact-result">{JSON.stringify({ local_only: telemetryInfo.local_only, counters: telemetryInfo.counters ?? {} }, null, 2)}</pre>}</div></article>
      <article className="card security-section approvals-section"><div className="card-heading"><div><p className="section-kicker">APPROVALS</p><h2>Ожидающие подтверждения</h2></div><span className="count-pill">{approvals.filter((item) => item.status === "pending").length}</span></div><div className="approval-list">{approvals.length === 0 ? <div className="compact-empty"><strong>Запросов нет</strong><span>Ничего не ожидает подтверждения.</span></div> : approvals.map((approval) => <article className="approval-row" key={approval.approval_id}><div><strong>{approval.tool || "operation"}</strong><small>{approval.target || "—"} · {approval.actor || "—"} · {approval.status || "unknown"}</small></div>{approval.status === "pending" && <div className="row-actions"><button className="text-button" type="button" disabled={busy} onClick={() => void decide(approval.approval_id || "", "approve")}>Разрешить</button><button className="text-button danger-text" type="button" disabled={busy} onClick={() => void decide(approval.approval_id || "", "reject")}>Отклонить</button></div>}</article>)}</div></article>
    </section>
  </div>;
}
