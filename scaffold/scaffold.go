...
var templateFiles = []templateFile{
	...
	// main_test.go is the agent-wide HTTP test — it builds the agent via
	// newAgent() and serves agent.Handler() so registered routes are tested
	// through the real mux. Scaffolded once (not regenerated by housekeeping);
	// the agent owns it and Sol extends it as it adds routes.
...
// NoticesFilename is the airlock-owned third-party notices file written into
// each agent. It is deliberately distinct from the conventional
// THIRD_PARTY_NOTICES.md so a user can keep their own notices there (for deps
// they add) without airlock's regeneration overwriting them.
const NoticesFilename = "THIRD_PARTY_NOTICES.generated.md"

// noticesTemplate is the embedded source for NoticesFilename. Copied verbatim
// (it is not a text/template — license bodies contain "{{").
const noticesTemplate = "templates/THIRD_PARTY_NOTICES.generated.md"
...
var staticFiles = []templateFile{
	// The notices file attributes the open-source the agent bundles (its Go
	// deps + the Tailwind/DaisyUI CSS compiled into the served stylesheet).
	// Generated by scripts/gen-notices.sh; required by those licenses to travel
	// with the distributed agent binary.
	{noticesTemplate, NoticesFilename},
}
...
// GenerateNotices writes the agent's third-party notices into dir as
// NoticesFilename. It's a verbatim copy of the embedded template (generated
// from the dep graph by scripts/gen-notices.sh), called at scaffold time and by
// the build pipeline's housekeeping so previously built agents pick up license
// updates. Airlock-owned — it never touches a user-authored
// THIRD_PARTY_NOTICES.md.
func GenerateNotices(dir string) error {
	b, err := templates.ReadFile(noticesTemplate)
	if err != nil {
		return fmt.Errorf("read notices template: %w", err)
	}
	if err := os.WriteFile(filepath.Join(dir, NoticesFilename), b, 0o644); err != nil {
		return fmt.Errorf("write %s: %w", NoticesFilename, err)
	}
	return nil
}
